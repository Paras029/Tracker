                    ...
                    modifier = Modifier.fillMaxWidth().clickable {
                        scope.launch {
                            runCatching {
                                ...
                                // only when a Web OAuth client exists in google-services.json.
                                // Look it up at runtime so the app compiles even without it.
                                val webClientIdRes = context.resources.getIdentifier(
                                    "default_web_client_id", "string", context.packageName
                                )
                                if (webClientIdRes == 0) return@runCatching
